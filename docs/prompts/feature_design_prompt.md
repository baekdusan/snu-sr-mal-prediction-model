...
- Your job is to design features based ONLY on query characteristics, not on observed MAL values.
- Design features that theoretically should predict MAL, based on your HFE and cognitive psychology knowledge.
...
- **Remember: You are NOT given MAL values. Design features based on theoretical understanding, not empirical observation of MAL.**
...
1. **Problem_Understanding**
   - 2–4 sentences restating the goal and dataset context.
   - Note that you are designing features WITHOUT access to MAL values (to prevent data leakage).
