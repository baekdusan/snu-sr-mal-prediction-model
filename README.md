...
├── quick_train.py                   # Quick baseline training (recommended)
├── predict_interactive.py           # Interactive prediction interface
...
├── best_improved_model.pkl          # Trained model (generated by quick_train.py)
...
**Example session:**
```
🔍 Enter query: 오늘 점심 메뉴 추천해줘
📊 Predicted MAL: 2.45 seconds
   → Fast response expected (< 4s)

🔍 Enter query: 지난달 카드 결제 내역 정리해서 보여줘
📊 Predicted MAL: 7.82 seconds
   → Moderate delay acceptable (4-8s)
...
- R² of 0.42 indicates moderate predictive power given small dataset size
